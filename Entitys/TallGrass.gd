...
func _ready():
	get_tree().current_scene.find_node("Player").find_node("MovementComponent").connect("entity_moving_signal", self, "player_exiting_grass")
	get_tree().current_scene.find_node("Player").find_node("MovementComponent").connect("entity_stopped_signal", self, "player_in_grass")
